...
External tracker access is read-only unless the user explicitly authorizes the specific state change, comment, label change, or closure. Gather and recommend freely; before a mutation, state the exact target and action and obtain that authorization.
...
These are canonical role names. Use the project-documented mapping when one exists; otherwise report the recommendation without changing labels.
